;(function( jQ, _ ){
  ...
  /*
    ...
    slow and inefficiently shares data. It aims to minimize the use of memory in an application by
    sharing as much data as possible with related objects.

    Two ways in which the Flyweight pattern can be applied:
      1) At the data-layer, where we deal with the concept of sharing data between large quantities of
        similar objects stored in memory.
      2) At the DOM-layer, where the Flyweight can be used as a central event-manager to avoid attaching
        event handlers to every child element in a parent container.
   */
  ...
  // Create a CoffeeFlavorFactory
  function CoffeeFlavorFactory() {
    var flavors = {},
        length  = 0;
    return {
      getCoffeeFlavor: function(name) {
  ...
  function testFlyweight() {
    ...
    function takeOrders( flavorIn ,table) {
      ...
      tables[ordersMade+=1] = new CoffeeOrderContext( table );
    }
    ...
    for (var i = 0; i < ordersMade; i+=1) (function(i){
      flavors[i].serveCoffee(tables[i]);
    })(i)

    console.log('total CoffeeFlavor Objects made: ' + flavorFactory.getTotalCoffeeMade() );
  }
